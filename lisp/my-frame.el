...
;; 	(face-height (face-attribute 'default :height)))
;;     (set-face-attribute 'default nil :height (+ face-height 5))
;;     (set-frame-size nil frame-width frame-height t))
(defun my-frame-increment-font-size (frame &optional arg)
  (let ((frame-width (frame-text-width frame))
	(frame-height (frame-text-height frame))
	(face-height (face-attribute 'default :height frame)))
    (let ((f (if (and arg (< arg 0)) '- '+))
	  (arg (if arg (abs arg) 1))
	  (face-height-current face-height)
	  (num 0))
      (while (< num arg)
	(setq face-height-current (apply f (list face-height-current 5)))
	(set-face-attribute 'default frame :height face-height-current)
	(setq num (1+ num))))
  ...
  (let* ((repeat t)
	...
	(cond ((eq key ?i)
	      ...
	      ((eq key ?l)
	       ...
	       (message "Reset to initial settings."))
	      ((eq key ?-)
	       (my-frame-increment-font-size nil -1))
	      ((eq key ?=)
	       (my-frame-increment-font-size nil +1))
	      ((eq key ?+)
	       (my-frame-increment-font-size nil +1))
